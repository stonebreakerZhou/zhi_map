...
- **两种视图。** 极简对话视图，以及按需进入的知识图谱视图，用于查看分支、关系和历史路径。
...
图谱视图可用但尚未完成：初始布局仍为插入顺序网格，而非层级星空树；完整子树展开、空间碰撞聚合、外框连续变形、访问历史与镜头往返、原生 WebView2／Windows 缩放验证仍有缺项。浏览器证据在 `test-results/`；Headless RAF 采样不代表已实测桌面 60fps。
