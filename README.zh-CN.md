...
- **开始与整理讨论**：新建主题，按标题或标签搜索，通过主题的“更多”入口重命名、收藏或移除。移除使用独立的十分钟服务端收据，无关草稿与问答不会使恢复失效；删除整个会话主线仍需单独确认，且不可撤销。
...
- **阅读排版后的回答**：回复按 Markdown 渲染——标题、列表、引用、加粗、行内代码与链接，公式由 KaTeX 排版：行内 `$...$` 或 `\(...\)`，独立成行 `$$...$$` 或 `\[...\]`。流式输出的文字在生成过程中同样即时渲染；公式选不中时，可在该条消息切换到“选择原文”后再框选。

## 开始使用

### 原生星空图实施状态

React 默认工作区已用 SVG 关系线和 HTML 对话胶囊呈现原生分支 ID。已接入专注／邻域／概览按钮、稳定编辑器、空白拖动平移、长按框选、连接柄／标题长按连线、显式位置整理，以及右键划删预览、释放提交和服务端恢复。文字选区、引用确认和模型设置继续复用现有组件。联系不进入 AI 上下文，引用线从既有引用记录派生。

`/api/graph` 限制为 200 个投影对象、400 条边、256 KiB。Alembic `0005_constellation` 在既有数据库新增布局、联系和独立移除记录。历史备份目前不包含图位置、联系和恢复收据；NDJSON 覆盖导入会清除这些图专用记录。

**目前尚未完整达到 QA-010 交付条件**：初始布局仍为插入顺序网格，尚非层级星空树；完整子树展开、空间碰撞聚合、活动节点外框连续变形、访问历史与镜头往返、原生 WebView2／Windows 缩放验证仍有缺项，INT-016 新增的主线轮次与子节点悬停预览也尚未完整验收。浏览器证据在 `test-results/graph-*.png` 和 `graph-measurements.json`。Headless RAF 采样不代表已实测桌面 60fps；最新便携 ZIP 已生成于 `desktop/dist/`，只有检测到 Inno Setup 时才生成安装器。
...
大型历史可使用 NDJSON 流式下载与磁盘暂存导入：单条记录 ≤ 1 MiB，文件 ≤ 2 GiB；旧 JSON 请求限制为 8 MiB。启动自动执行 Alembic 升级，原工作区 JSON 保留为迁移时备份。迁移较大工作区前请阅读[架构与当前扩展限制](docs/architecture.zh-CN.md)。私有模型地址须由服务端显式设置 `AI_ALLOW_PRIVATE_HOSTS=true`，生产模式仍要求 HTTPS。使用 Clash／Mihomo fake-IP 代理时，`AI_FAKE_IP_HOSTS`（逗号分隔的精确主机名，仅限 HTTPS 标准端口）会额外接受代理解析出的此类虚拟地址；该值在独立后端默认为空，Windows 桌面版默认设为 `api.deepseek.com`。裸 IP、其他私网地址、非标准端口及其余地址规则仍然生效。
