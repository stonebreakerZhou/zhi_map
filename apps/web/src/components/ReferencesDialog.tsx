...
export function ReferencesDialog({ app, close, initialSource }: { app: WorkspaceController; close: () => void; initialSource?: string }) {
  const target = app.branch!;
  const [search, setSearch] = useState(''), [topicCursor, setTopicCursor] = useState(-1), [cursor, setCursor] = useState(-1), [source, setSource] = useState(initialSource ?? '');
  ...
  return <Dialog title="只带入需要的前提" close={close} label={target.pendingPrompt ? '确认引用并继续' : '确认加入引用'} disabled={!source || !selected.size || [...selected.values()].some(r => !Number.isInteger(r.start) || !Number.isInteger(r.end) || r.start < 0 || r.end <= r.start)} confirm={async () => { await app.action({ type: target.pendingPrompt ? 'resolveHistory' : 'reference', decision: 'reference', branchId: target.id, sourceId: source, selectedIds: [...selected.keys()], ranges: Object.fromEntries(selected) }); if (app.branch?.awaiting) void app.ask().catch(app.report); }}>
    <label className="field">搜索来源主题<input maxLength={120} value={search} onChange={e => { setSearch(e.target.value); setTopicCursor(-1); }} /></label>
    <Pager cursor={topicCursor} next={topics.page.nextCursor} change={setTopicCursor} /><label className="field">来源主题<select id="source-select" value={source} onChange={e => choose(e.target.value)}><option value="">选择来源</option>{source && !topics.page.items.some(b => b.id === source) && <option value={source}>已选关联来源</option>}{topics.page.items.filter(b => b.id !== target.id).map(b => <option key={b.id} value={b.id}>{b.title}</option>)}</select></label>
