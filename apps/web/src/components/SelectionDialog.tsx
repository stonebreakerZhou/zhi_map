...
export function SelectionDialog({ app, selection, close }: { app: WorkspaceController; selection: Selection; close: () => void }) {
  ...
  return <Dialog title="沿着这一点，继续想" close={cancel} dirty={Boolean(text.trim())} label="展开讨论" disabled={!text.trim() || !selection.text.trim()} confirm={async () => { await app.action({ type: 'expand', branchId: bid, selection, contextScope: { mode: all ? 'all' : 'none', excludedIds: [...excluded] }, text }); if (app.branch?.awaiting) void app.ask().catch(app.report); }}>
